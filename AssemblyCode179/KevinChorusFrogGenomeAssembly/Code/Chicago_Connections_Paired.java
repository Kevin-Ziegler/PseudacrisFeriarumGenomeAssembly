...
public class Chicago_Connections_Paired{
	...
	public static void main(String args[]){
		...
		String inputFiles_HiC_Paired = args[1];
		String inputFile_SingleCopyKmers_PBs = args[2];
		...
		try{
			...
			System.out.println("Loaded SC Kmers into Hash");			
			...
			String[] files_Paired = inputFiles_HiC_Paired.split("__");
			int lineNum = 0;
			String line_Paired;
			...
			for(int f = 0; f < files.length; f++){
				...
				FileReader hiCF_Paired = new FileReader(files_Paired[f]);
				BufferedReader hiCBR_Paired = new BufferedReader(hiCF_Paired);

				line = hiCBR.readLine();
				line = hiCBR.readLine();
				line_Paired = hiCBR_Paired.readLine();
				line_Paired = hiCBR_Paired.readLine();
				...
				while(line != null){
					...
					ArrayList<ArrayList<Integer>> tempPbIds_Paired = new ArrayList<ArrayList<Integer>>();
					...
					for(int i = 0; i < (line_Paired.length()-19); i++){
						String tempKmer = line_Paired.substring(i, i+19);
						//System.out.println(tempKmer);
						if(map.containsKey(tempKmer)){
							ArrayList<Integer> tempAL = map.get(tempKmer);
							tempPbIds_Paired.add(tempAL);
							//System.out.println(tempAL);
							//locationTempPbIds.add(i);
						}
						if(rCmap.containsKey(tempKmer)){
					...
					if(tempPbIds_Paired.size() > 0){
  		                        	for(int i = tempPbIds_Paired.size()-1; i >= 0; i--){
							tempPbIds.add(tempPbIds_Paired.get(i));
						}
					}
